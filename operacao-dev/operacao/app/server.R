...
server <- function(input, output, session) {
  ...
  outTable <- function(dados){
    ...
    output$table <-  DT::renderDataTable(datatable(dados, filter = 'top', class = 'cell-border stripe', width = '100%', height = '100%', 
                                                   extensions = c('FixedHeader', 'Buttons'),
                                         options = list(fixedHeader = TRUE, pageLength = nrow(dados), dom = 'Bfrtip',
                                                        buttons = c('copy', 'csv', 'excel', 'pdf',  I('colvis'))), rownames = FALSE) %>%
