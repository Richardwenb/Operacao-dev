/* @(#) $Id: //bas/753_REL/src/include/sapuc.h#5 $ SAP*/
/* CCQ_CCU_FILE_OFF */ /* CCQ_COV_FILE_OFF */ /* CCQ_IPV6_SUPPORT_FILE_OK */ /* CCQ_USE_TIME_T_FILE_OK */
#ifndef SAPUC_H
#define SAPUC_H "$Id: //bas/753_REL/src/include/sapuc.h#5 $"
...
/*-HISTORY----------------------------------------------------------------------
 * 28.11.2016 bs       add strerror_rU()
 * 18.06.2013 hh       remove SCCSID (sccsidU and SAP_IDENT should be used)
 * 14.03.2013 hh       remove mktempU (also for os390)
 * 06.02.2013 jk       remove tmpnamU (also for as400)
 * 04.02.2013 hh       remove mktempU (all platforms but rs6000_64, as400, os390)
 *                     remove tmpnamU (all platforms but as400)
 *                     remove tempnamU (all platforms)
 *                     please use mkstempU instead !
 * 02.10.2012 tm       add SAP_CESU8.
 * 16.08.2012 tm       stop using wchar_t for SAP_U4 and use SA_UINT for all the
 *                     platforms.
 ...
 *     #define SAP_IDENT "@(#) $Id: //bas/753_REL/src/include/sapuc.h#5 $"
 ...
 *     SAP_IDENTN(name,"@(#) $Id: //bas/753_REL/src/include/sapuc.h#5 $")
...
#define strlenR( s )                /*SAPUNICODEOK_LIBFCT*/ /*SAPUNICODEOK_CHARTYPE*/ strlen( (const char*) (s) )
...
#define strchrR( s, c )             /*SAPUNICODEOK_LIBFCT*/ /*SAPUNICODEOK_CHARTYPE*/ (SAP_RAW*) strchr( (const char*) (s), c )
#define strrchrR( s, c )            /*SAPUNICODEOK_LIBFCT*/ /*SAPUNICODEOK_CHARTYPE*/ (SAP_RAW*) strrchr( (const char*) (s), c )
...
#if defined(SAPonNT)
    /* allow SAP implementation on Windows */
    #undef memcmpU16
#endif
...
/*----------------------------------------------------------------------------
 * sapuc.h - fortify source functions
 *----------------------------------------------------------------------------*/

#ifdef SAP_FORTIFY_SOURCE
#undef SAP_FORTIFY_SOURCE
#endif

/* activate fortify source implementation */
#define SAP_FORTIFY_SOURCE

#if (defined(SAPonLIN) && defined(__GNUC__) && defined(SAP_FORTIFY_SOURCE) && !defined(SAPccQ) && defined(__x86_64__) && !defined(NDEBUG))
/* #warning # SAP_FORTIFY_SOURCE is activated */

/* strnlen() required SAP_FORTIFY_SOURCE */
externC size_t strnlen(const char *s, size_t maxlen);

    #if 1
        /* inline fortify source functions to reduce dependencies.
         * the caller information of the mem/str*Fortify function is
         * printed as 'ERROR: FORTIFY_SOURCE ... [_FILE_:_LINE_]
         * in the str/mem*Fortify function
         */
        #define SAP_STATIC_INLINE  static inline        \
            __attribute__((__always_inline__))          \
            __attribute__((__unused__))
        #define SAP_FORTIFY_SOURCE_INLINED
    #else
        /* if the *Fortify functions are not inlined they will be shown in the stack
         * but this requires linking of the str/mem functions used inside the *Fortify
         * functions. unfortunately this is not the case in all projects ...
         */
        #define SAP_STATIC_INLINE  static       \
    ...
    #ifdef SAP_DPTRCLIB
        #ifdef __cplusplus
            extern "C" SAP_INT CTrcStack (FILE *fp);
        #else
            SAP_INT CTrcStack (FILE *fp);
        #endif

        #define SAP_FORTIFY_SOURCE_TRC_STACK(fp)                                        \
        do {                                                                            \
          CTrcStack(fp);                                                                \
          fprintf (fp, "ERROR: FORTIFY_SOURCE --> "                                     \
                 "https://wiki.wdf.sap.corp/wiki/display/LinuxLab/Fortify+Source\n");   \
        } while (0)
    #else
        /* weak binding of CTrcStack() to support make targets
         * which do not link the dptrc library. they will detect a
         * overflow and print a statement but can not trace the call stack
         */
        #ifdef __cplusplus
            extern "C" SAP_INT CTrcStack (FILE *fp) __attribute__((__weak__));
        #else
            SAP_INT CTrcStack (FILE *fp) __attribute__((__weak__));
        #endif
        ...
        do {                                                                            \
          ...
          {                                                                             \
            fprintf (fp, "CTrcStack() function is not available, please link "          \
                         "dptrclib.a to get a full stack trace\n");                     \
          }                                                                             \
          fprintf (fp, "ERROR: FORTIFY_SOURCE --> "                                     \
                   "https://wiki.wdf.sap.corp/wiki/display/LinuxLab/Fortify+Source\n"); \
        } while (0)
...
SAP_STATIC_INLINE
       SAP_INT memchkFortify (void *dst, const void *src,
                              size_t len, const char *func,
                              const char *file, int line)
{
    if (((SAP_RAW*) dst < (SAP_RAW*) src && ((SAP_RAW*) dst) + len > (SAP_RAW*) src ) ||
        ((SAP_RAW*) src < (SAP_RAW*) dst && ((SAP_RAW*) src) + len > (SAP_RAW*) dst ) ||
        (src == dst)                       )
    {
        fprintf(stderr,"%s overlapping - from: %p to: %p len: %d (%s:%d)\n",
                func, src, dst, (int)len, file, line);

        if (getenv("SAPMEMCPYCHK") && !strcmp(getenv("SAPMEMCPYCHK"),"0"))
            return 2;
...
/* the mem/str*Foritfy functions implement
 * a simple check of the length of the destination buffer.
 * in case of an overflow, they just report an error,
 * print the call stack and run str/mem*R/U function like before.
 * well, even if we know that we detected an overflow
 * we do not want to change/fix the code and the resulting behaviour!
 */

SAP_STATIC_INLINE
       void* memsetFortify  (void *dst, int c, size_t len,
                             size_t dst_len, int *rc, const char *file, int line )
{
    fprintf (stderr, "ERROR: FORTIFY_SOURCE - memset, dst_len (%lu) < len (%lu) [%s:%d]\n",
             dst_len, len, file, line);

    SAP_FORTIFY_SOURCE_TRC_STACK (stderr);
...
SAP_STATIC_INLINE
       void* memmoveFortify (void *dst, const void *src, size_t len,
                             size_t dst_len, int *rc, const char *file, int line )
{
    fprintf (stderr, "ERROR: FORTIFY_SOURCE - memmove, dst_len (%lu) < len (%lu) [%s:%d]\n",
             dst_len, len, file, line);
...
SAP_STATIC_INLINE
       void* memcpyFortify (void *dst, const void *src, size_t len,
                            size_t dst_len, int *rc, const char *file, int line )
{
    fprintf (stderr, "ERROR: FORTIFY_SOURCE - memcpy, dst_len (%lu) < len (%lu) [%s:%d]\n",
             dst_len, len, file, line);
...
#ifdef MEMCPY_CHK_FORTIFY
    memchkFortify (dst, src, len, cR("memcpyR"), file, line);
    //return memcpyRChk (dst, src, len, (SAP_RAW*) file, line);
#endif
    return memcpy (dst, src, len);
}

SAP_STATIC_INLINE
       char* strcatFortify (char *dst, const char *src, size_t obj_len,
                            int *rc, const char *file, int line )
{
    SAP_UINT    dst_len, src_len;

    dst_len = strlen (dst);
    src_len = strlen (src);

    /* check required destination buffer length (inclusive string termination) */
    if ((src_len + dst_len + 1) <= obj_len)
        return strcat (dst, src);

    /* !!! overflow !!! */
    fprintf (stderr, "ERROR: FORTIFY_SOURCE - strcat, dst_size (%lu) <= "
                     "strlenR(dst) + strlenR(src) (%u + %u) [%s:%d]\n",
             obj_len, dst_len, src_len, file, line);
...
SAP_STATIC_INLINE
       char* strncatFortify (char *dst, const char *src, size_t len,
                             size_t obj_len, int *rc, const char *file, int line )
{
    SAP_UINT    dst_len, src_len;
    ...
    /* check required destination buffer length (inclusive string termination) */
    if ((dst_len + src_len + 1) <= obj_len)
        return strncat (dst, src, len);

    /* !!! overflow !!! */
    fprintf (stderr, "ERROR: FORTIFY_SOURCE - strncat, dst_size (%lu) <= "
                     "strlenR(dst) + n/strlenR(src) (%u + %lu/%u) [%s:%d]\n",
             obj_len, dst_len, len, src_len, file, line);
...
SAP_STATIC_INLINE
       char* strcpyFortify (char *dst, const char *src, size_t obj_len,
                            int *rc, const char *file, int line )
{
    SAP_UINT    src_len;
    ...
    /* check required destination buffer length (inclusive string termination) */
    if ((src_len + 1) <= obj_len)
        return strcpy (dst, src);

    /* !!! overflow !!! */
    fprintf (stderr, "ERROR: FORTIFY_SOURCE - strcpy, dst_size (%lu) <= "
                     "strlenR(src) (%u) [%s:%d]\n",
             obj_len, src_len, file, line);
...
SAP_STATIC_INLINE
       void* memsetU16Fortify  (SAP_UTF16 *dst, int c, size_t len,
                                size_t dst_len, int *rc, const char *file, int line )
{
    fprintf (stderr, "ERROR: FORTIFY_SOURCE - memsetU, dst_len (%lu) < len (%lu) [%s:%d]\n",
             dst_len, len * SAP_UC_LN, file, line);

    SAP_FORTIFY_SOURCE_TRC_STACK (stderr);

    if (rc != NULL) *rc = -1;

    return SAP_UNAME_MEM_RET(memset)( dst, U_CHAR(c), len);
}

SAP_STATIC_INLINE
       void* memmoveU16Fortify (void *dst, const void *src, size_t len,
                                size_t dst_len, int *rc, const char *file, int line )
{
    fprintf (stderr, "ERROR: FORTIFY_SOURCE - memmoveU, dst_len (%lu) < len (%lu) [%s:%d]\n",
             dst_len, len * SAP_UC_LN, file, line);
    ...
    return SAP_UNAME_MEM_RET(memmove)( dst, src, len);
}

SAP_STATIC_INLINE
       void* memcpyU16Fortify (SAP_UTF16 *dst, const SAP_UTF16 *src, size_t len,
                               size_t dst_len, int *rc, const char *file, int line )
{
    fprintf (stderr, "ERROR: FORTIFY_SOURCE - memcpyU, dst_len (%lu) < len (%lu) [%s:%d]\n",
             dst_len, len * SAP_UC_LN, file, line);
...
#ifdef MEMCPY_CHK_FORTIFY
    memchkFortify (dst, src, len, cR("memcpyU"), file, line);
    //return memcpyUChk (dst, src, len, (SAP_RAW*) file, line);
#endif
    return SAP_UNAME_MEM_RET(memcpy)( dst, src, len);
}

BEGIN_NS_STD_C_HEADER

SAP_STATIC_INLINE
       SAP_UTF16* strcatU16Fortify (SAP_UTF16 *dst, const SAP_UTF16 *src, size_t obj_len,
                                    int *rc, const char *file, int line )
{
    const SAP_UTF16    *p;
    SAP_UINT            dst_len, src_len;
    ...
    /* check required destination buffer length inclusive string termination */
    if ((src_len + dst_len + 1) * SAP_UC_LN <= obj_len)
        return SAP_UNAME(strcat) (dst, src);

    /* !!! overflow !!! */
    fprintf (stderr, "ERROR: FORTIFY_SOURCE - strcatU, dst_size (%lu) <= "
                     "strlenU(dst) + strlenU(src) (%u + %u) [%s:%d]\n",
             obj_len, dst_len * SAP_UC_LN, src_len * SAP_UC_LN, file, line);
...
SAP_STATIC_INLINE
       SAP_UTF16* strncatU16Fortify (SAP_UTF16 *dst, const SAP_UTF16 *src, size_t len,
                                     size_t obj_len, int *rc, const char *file, int line )
{
    SAP_UINT            dst_len = 0, src_len = 0;

    while (dst[dst_len] != cU('\0'))
        dst_len++;

    while (src[src_len] != cU('\0') && src_len < len)
        src_len++;

    /* check required destination buffer length inclusive string termination */
    if ((src_len + dst_len + 1) * SAP_UC_LN <= obj_len)
        return SAP_UNAME(strncat) (dst, src, len);
        // return strncatU16 (dst, src, len);

    /* !!! overflow !!! */
    fprintf (stderr, "ERROR: FORTIFY_SOURCE - strncatU, dst_size (%lu) <= "
                     "strlenU(dst) + n/strlenU(src) (%u + %lu/%u) [%s:%d]\n",
             obj_len, dst_len * SAP_UC_LN, len * SAP_UC_LN,
             src_len * SAP_UC_LN, file, line);
...
SAP_STATIC_INLINE
       SAP_UTF16* strcpyU16Fortify (SAP_UTF16 *dst, const SAP_UTF16 *src, size_t obj_len,
                                    int *rc, const char *file, int line )
{
    const SAP_UTF16    *p;
    SAP_UINT            src_len;
    ...
    /* check required destination buffer length inclusive string termination */
    if ((src_len + 1) * SAP_UC_LN <= obj_len)
        return SAP_UNAME(strcpy) (dst, src);
        // return strcpyU16 (dst, src);

    /* !!! overflow !!! */
    fprintf (stderr, "ERROR: FORTIFY_SOURCE - strcpyU, dst_size (%lu) <= "
                     "strlenU(src) (%u) [%s:%d]\n",
             obj_len, src_len * SAP_UC_LN, file, line);
...
/* fortify source macro definitions
 * - replace mem*[RU] and str*[RU] macros to enforce a overflow check
 *   based on the gcc builtin_object_size feature
 * - the *_chk macros support a 'return code' parameter to verify the result of the check
 */

/* avoid some annoying gcc-4.3 warnings via compare in inline function */
SAP_STATIC_INLINE SAP_INT lessEqualFortify (size_t a, size_t b)
{
  return a <= b;
}

    #define memsetR_chk(t,c,l,rc)                                                                \
        /*CCQ_SECURE_LIB_OK*/ /*CCQ_NUM_EXPR_OK*/ /*SAPUNICODEOK_LIBFCT*/                        \
        ((__builtin_object_size (t, 0) == (size_t) -1 ||                                         \
          lessEqualFortify (l, __builtin_object_size (t, 0)) ) ?	\
            memset(t,c,l) :                                                                      \
            memsetFortify(t,c,l,__builtin_object_size (t, 0),rc,cR(__FILE__),__LINE__))
    #undef memsetR
    #define memsetR(t,c,l)      memsetR_chk(t,c,l,NULL)

    #define memmoveR_chk(t,s,l,rc)                                                               \
        /*CCQ_SECURE_LIB_OK*/ /*CCQ_NUM_EXPR_OK*/ /*SAPUNICODEOK_LIBFCT*/                        \
        ((__builtin_object_size (t, 0) == (size_t) -1  ||                                        \
          lessEqualFortify (l, __builtin_object_size (t, 0)) ) ?	\
            memmove(t,s,l) :                                                                     \
            memmoveFortify(t,s,l,__builtin_object_size (t, 0),rc,cR(__FILE__),__LINE__))
    #undef memmoveR
    #define memmoveR(t,s,l)     memmoveR_chk(t,s,l,NULL)

  #ifdef MEMCPY_CHK_FORTIFY
    /* use memcpyRChk to check if src/dst do overlap */
    #define memcpyR_chk(t,s,l,rc)                                                                \
        /*CCQ_SECURE_LIB_OK*/ /*CCQ_NUM_EXPR_OK*/ /*SAPUNICODEOK_LIBFCT*/                        \
        ((__builtin_object_size (t, 0) == (size_t) -1 ||                                         \
          lessEqualFortify (l, __builtin_object_size (t, 0)) ) ?	\
            memcpyRChk(t,s,l,(SAP_RAW*)cR(__FILE__),__LINE__) :                                  \
            memcpyFortify(t,s,l,__builtin_object_size (t, 0),rc,cR(__FILE__),__LINE__))
  #else
    #define memcpyR_chk(t,s,l,rc)                                                                \
        /*CCQ_SECURE_LIB_OK*/ /*CCQ_NUM_EXPR_OK*/ /*SAPUNICODEOK_LIBFCT*/                        \
        ((__builtin_object_size (t, 0) == (size_t) -1 ||                                         \
    ...
    #undef memcpyR
    #define memcpyR(t,s,l)      memcpyR_chk(t,s,l,NULL)

    /* if builtin_object_size is known the strlen check is done in strcatFortify */
    #define strcatR_chk(t,s,rc)                                                                  \
        /*CCQ_SECURE_LIB_OK*/ /*CCQ_NUM_EXPR_OK*/ /*SAPUNICODEOK_LIBFCT*/                        \
        ((__builtin_object_size (t, 0) == (size_t) -1) ?                                         \
        strcat(t,s) :                                                                            \
        strcatFortify(t,s,__builtin_object_size (t, 0),rc,cR(__FILE__),__LINE__))
    #undef strcatR
    #define strcatR(t,s)        strcatR_chk(t,s,NULL)

    /* if builtin_object_size is known the strlen check is done in strncatFortify */
    #define strncatR_chk(t,s,n,rc)                                                               \
        /*CCQ_SECURE_LIB_OK*/ /*CCQ_NUM_EXPR_OK*/ /*SAPUNICODEOK_LIBFCT*/                        \
        ((__builtin_object_size (t, 0) == (size_t) -1) ?                                         \
        strncat(t,s,n) :                                                                         \
        strncatFortify(t,s,n,__builtin_object_size (t, 0),rc,cR(__FILE__),__LINE__))
    #undef strncatR
    #define strncatR(t,s,n)     strncatR_chk(t,s,n,NULL)

    /* if builtin_object_size is known the strlen check is done in strcpyFortify */
    #define strcpyR_chk(t,s,rc)                                                                  \
        /*CCQ_SECURE_LIB_OK*/ /*CCQ_NUM_EXPR_OK*/ /*SAPUNICODEOK_LIBFCT*/                        \
        ((__builtin_object_size (t, 0) == (size_t) -1) ?                                         \
        strcpy(t,s) :                                                                            \
        strcpyFortify(t,s,__builtin_object_size (t, 0),rc,cR(__FILE__),__LINE__))
    #undef strcpyR
    #define strcpyR(t,s)        strcpyR_chk(t,s,NULL)

#ifdef SAPwithUNICODE
    #define memsetU_chk(t,c,l,rc)                                                                \
        /*CCQ_SECURE_LIB_OK*/ /*CCQ_NUM_EXPR_OK*/ /*SAPUNICODEOK_LIBFCT*/                        \
        ((__builtin_object_size (t, 0) == (size_t) -1              ||                            \
          __builtin_object_size (t, 0) >= (size_t) (l) * SAP_UC_LN    ) ?                        \
            memsetU16(t,c,l) :                                                                   \
            (SAP_UTF16*)memsetU16Fortify(t,c,l,__builtin_object_size (t, 0),rc,cR(__FILE__),__LINE__))
    #undef memsetU
    #define memsetU(t,c,l)      memsetU_chk(t,c,l,NULL)

    #define memmoveU_chk(t,s,l,rc)                                                               \
        /*CCQ_SECURE_LIB_OK*/ /*CCQ_NUM_EXPR_OK*/ /*SAPUNICODEOK_LIBFCT*/                        \
        ((__builtin_object_size (t, 0) == (size_t) -1              ||                            \
          __builtin_object_size (t, 0) >= (size_t) (l) * SAP_UC_LN    ) ?                        \
            memmoveU16(t,s,l) :                                                                  \
            (SAP_UTF16*)memmoveU16Fortify(t,s,l,__builtin_object_size (t, 0),rc,cR(__FILE__),__LINE__))
    #undef memmoveU
    #define memmoveU(t,s,l)     memmoveU_chk(t,s,l,NULL)

  #ifdef MEMCPY_CHK_FORTIFY
    /* internal check if src/dst do overlap */
    #define memcpyU_chk(t,s,l,rc)                                                                \
        /*CCQ_SECURE_LIB_OK*/ /*CCQ_NUM_EXPR_OK*/ /*SAPUNICODEOK_LIBFCT*/                        \
        /*CCQ_SECURE_LIB_OK*/ /*CCQ_NUM_EXPR_OK*/ /*SAPUNICODEOK_LIBFCT*/                        \
        ((__builtin_object_size (t, 0) == (size_t) -1              ||                            \
          __builtin_object_size (t, 0) >= (size_t) (l) * SAP_UC_LN    ) ?                        \
            memcpyUChk(t,s,l,(SAP_RAW*)cR(__FILE__),__LINE__) :                                  \
            (SAP_UTF16*)memcpyU16Fortify(t,s,l,__builtin_object_size (t, 0),rc,cR(__FILE__),__LINE__))
  #else
    /* call memcpyUchk to check if src/dst do overlap */
    #define memcpyU_chk(t,s,l,rc)                                                                \
        /*CCQ_SECURE_LIB_OK*/ /*CCQ_NUM_EXPR_OK*/ /*SAPUNICODEOK_LIBFCT*/                        \
        ((__builtin_object_size (t, 0) == (size_t) -1              ||                            \
          __builtin_object_size (t, 0) >= (size_t) (l) * SAP_UC_LN    ) ?                        \
            memcpy(t,s,(l)*SAP_UC_LN) :                                                          \
            (SAP_UTF16*)memcpyU16Fortify(t,s,l,__builtin_object_size (t, 0),rc,cR(__FILE__),__LINE__))
  #endif
    #undef memcpyU
    #define memcpyU(t,s,l)      memcpyU_chk(t,s,l,NULL)

    /* if builtin_object_size is known the strlen check is done in strcatU16Fortify */
    #define strcatU_chk(t,s,rc)                                                                  \
        /*CCQ_SECURE_LIB_OK*/ /*CCQ_NUM_EXPR_OK*/ /*SAPUNICODEOK_LIBFCT*/                        \
        ((__builtin_object_size (t, 0) == (size_t) -1) ?                                         \
        strcatU16(t,s) :                                                                         \
        strcatU16Fortify(t,s,__builtin_object_size (t, 0),rc,cR(__FILE__),__LINE__))
    #undef strcatU
    #define strcatU(t,s)        strcatU_chk(t,s,NULL)

    /* if builtin_object_size is known the strlen check is done in strncatU16Fortify */
    #define strncatU_chk(t,s,n,rc)                                       \
        /*CCQ_SECURE_LIB_OK*/ /*CCQ_NUM_EXPR_OK*/ /*SAPUNICODEOK_LIBFCT*/                        \
        ((__builtin_object_size (t, 0) == (size_t) -1) ?                                         \
        strncatU16(t,s,n) :                                                                      \
        strncatU16Fortify(t,s,n,__builtin_object_size (t, 0),rc,cR(__FILE__),__LINE__))
    #undef strncatU
    #define strncatU(t,s,n)     strncatU_chk(t,s,n,NULL)

    /* if builtin_object_size is known the strlen check is done in strcpyU16Fortify */
    #define strcpyU_chk(t,s,rc)                                                                  \
        /*CCQ_SECURE_LIB_OK*/ /*SAPUNICODEOK_LIBFCT*/                                            \
        ((__builtin_object_size (t, 0) == (size_t) -1) ?                                         \
        strcpyU16(t,s) :                                                                         \
        strcpyU16Fortify(t,s,__builtin_object_size (t, 0),rc,cR(__FILE__),__LINE__))
    #undef strcpyU
    #define strcpyU(t,s)        strcpyU_chk(t,s,NULL)

#else /* SAPwithUNICODE */

    #undef memsetU
    #define memsetU(t,c,l)      memsetR(t,c,l)

    #undef memmoveU
    #define memmoveU(t,s,l)     memmoveR(t,s,l)

    #undef memcpyU
    #define memcpyU(t,s,l)      memcpyR(t,s,l)

    #undef strcatU
    #define strcatU(t,s)        strcatR(t,s)

    #undef strncatU
    #define strncatU(t,s,n)     strncatR(t,s,n)

    #undef strcpyU
    #define strcpyU(t,s)        strcpyR(t,s)
#endif

#endif /* SAP_FORTIFY_SOURCE */
...
/* The scanf_s non-Unicode fallback functions are not yet implemented */
...
/*sscanf_sU*/
/* sscanf_sU is not yet implemented. */
 ...
 * BEGIN_NS_STD_C_HEADER
 *  SAP_U16_PROTOTYPE_FLIKE(sscanf_s)    /o SCANFLIKE2 o/;
 * END_NS_STD_C_HEADER
